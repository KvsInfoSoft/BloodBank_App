...
function donorRecord() {
  ...
  const [drpDonationType, setdrpDonationType] = useState([]);
  ...
  const onDonationType = () => {
    donorServices.getDonationType().then((response) => {
      try {
        if (response.data.statusCode === 200) {
          setdrpDonationType(response.data.data);
        }
        setLoading(false);
      } catch (e) {
  ...
  const ondonorRecordDetails = (params) => {
    setLoading(true);
    donorServices.getDonorDetails(params).then((response) => {
      try {
        if (response.data.statusCode === 200) {
                ...
                <Form.Group className="mb-3" as={Col} md="2" controlId="validationCustom02">
                  ...
                  <InputGroup className="mb-3">
                    <FormControl
                      ...
                      onChange={(event) => handleUserDrp({ target: { name: 'donationType', value: event.target.value } })}
                    >
                      <option className="mb-0 text-muted" value="0">
                        All
                      </option>
                      {drpDonationType.map((option) => (
                        <option key={option.donationType} value={option.donationType}>
                          {option.donationType}
                        </option>
                      ))}
                    </FormControl>
      ...
      <Row>
        <Col md={6} xl={12}>
          <Card className="user-list">
            <Card.Header>
              <Card.Title as="h5">Total Donor Record List : {totalRecord ? totalRecord : 0}</Card.Title>
            </Card.Header>
